...
/* ── Konfigurasi ─────────────────────────────────────────── */
/* GANTI dengan URL Web App hasil deploy Apps Script yang baru.
   Langkahnya ada di SETUP-SPREADSHEET.md. Spreadsheet lama sudah ditutup Google,
   jadi URL lama di bawah ini tidak dipakai lagi. */
const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbz-6NOrf0NjBGihBaTCcmEhnOwQNK6MTIKhwQsPo9hmotwXBv_fRNF1cGSvIpX0w0SgBw/exec';
...
/* ── Tangkap UTM dari URL, simpan selama sesi ──────────────
   Dipakai untuk membedakan lead dari Google Ads dan Meta Ads
   di spreadsheet. Kalau pengunjung datang tanpa UTM, kolomnya
   terisi 'direct'. */
function getUtm() {
  const KEY = 'pf_utm';
  try {
    ...
    if (fresh.utm_source || fresh.gclid) {
...
(function initForm() {
  ...
  form.addEventListener('submit', (e) => {
    ...
    const payload = {
      ...
      utm_source:   utm.utm_source || (utm.gclid ? 'google' : 'direct'),
      utm_medium:   utm.utm_medium || (utm.gclid ? 'cpc' : ''),
